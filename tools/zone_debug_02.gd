...
## Gerador de SVG de debug por zona da FASE 02 (headless, ZERO GPU).
##
...
##
## Roda headless:
##   Godot --headless --path . res://tools/zone_debug_02.tscn -- --area=all
## Args: --area=all|z1|z2|z3|z4|boss   (default all)

const _STAGE_SCENE := preload("res://stages/stage_02/stage_02.tscn")
const _OUT_DIR := "res://docs/stage_02/zones/"
...
const _AREA_TITLE := {
	"Z1": "Z1 — gelo escorregadio", "Z2": "Z2 — espinhos de gelo + miniboss",
	"Z3": "Z3 — nevasca", "Z4": "Z4 — abismos mortais", "Boss": "Boss — Cryovex (pendente)",
}
...
func _run() -> void:
	...
	_write_legend()
	_write_index()
	print("zone_debug_02: concluído. %d SVG + legenda + index em %s" % [made, _OUT_DIR])
