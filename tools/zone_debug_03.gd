...
## Gerador de SVG de debug por zona da FASE 03 (headless, ZERO GPU).
##
...
##
## Roda headless:
##   Godot --headless --path . res://tools/zone_debug_03.tscn -- --area=all
## Args: --area=all|z1|z2|z3|z4|boss   (default all)

const _STAGE_SCENE := preload("res://stages/stage_03/stage_03.tscn")
const _OUT_DIR := "res://docs/stage_03/zones/"
...
const _AREA_TITLE := {
	"Z1": "Z1 — condutores (trilhos eletrificados)", "Z2": "Z2 — trilhos (plataformas móveis)",
	"Z3": "Z3 — tempestade (raios)", "Z4": "Z4 — capacitores", "Boss": "Boss — Voltrix",
}
...
func _run() -> void:
	...
	_write_legend()
	_write_index()
	print("zone_debug_03: concluído. %d SVG + legenda + index em %s" % [made, _OUT_DIR])
