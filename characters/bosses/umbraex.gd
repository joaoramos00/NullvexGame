...
const _TEX_IDLE_NEW_E := preload("res://characters/bosses/umbraex/umbraex_idle_east.png")
const _TEX_HURT_NEW_E := preload("res://characters/bosses/umbraex/umbraex_hurt_east.png")
const _TEX_DEATH_NEW_E := preload("res://characters/bosses/umbraex/umbraex_death_east.png")
...
const _HURT_NEW_FRAMES  := 7
const _HURT_NEW_FPS     := 14.0
...
func _update_animation(delta: float) -> void:
	# DEATH override — trava tudo, toca uma vez, segura no ultimo frame.
	# Continua ticando mesmo apos is_dead (por isso nao ha o early return
	# antigo em state DYING/DEAD).
	...
	# HURT priority — antes dos ataques (mas nao interrompe attack ja em andamento,
	# so aparece quando NAO ta atacando pra evitar cortar visual do ataque)
	if _hurt_timer > 0.0 and _attack_anim == AttackAnim.NONE:
		_apply_new_anim_east(_TEX_HURT_NEW_E, _HURT_NEW_FRAMES, _HURT_NEW_FPS, delta, false)
		return
	...
	if absf(velocity.x) > 10.0:
		...
		# IDLE novo animado (substitui o static frame antigo)
		_apply_new_anim_east(_TEX_IDLE_NEW_E, _IDLE_NEW_FRAMES, _IDLE_NEW_FPS, delta, false)

# Helper pras anims novas (east-only + flip_h). Usa hold_last=true pra death
# (segura no ultimo frame). Outras anims (idle, hurt) loopam ou seguram
# conforme apply usual do _apply_anim.
